...
export const listenToWorkerEvents = (callback: (event: MinecraftClientEvent) => void) => {
...
>(
  ...
  return new Promise((resolve) => {
    const unsubscribe = listenToWorkerEvent(responseType, (responseEvent) => {
      ...
      resolve(responseEvent as Extract<MinecraftClientEvent, { type: TResponseEventType }>)
      unsubscribe()
    })
